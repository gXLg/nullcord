...
class Rest {
  ...
  async wrequest(options, scopePath, body){
    while(true){
      ...
      if (result.code != -1 || !this.retry) return result;
    }
  }
      ...
      for(const bucket in limit){
        if(limit[bucket].scopes.has(scope)){
          ...
          if(limit[bucket].remaining == 0){
            await new Promise(
              r => setTimeout(r,
                limit[bucket].reset[0] * 1000 - Date.now() + 700
              )
            );
          }
          break;
      ...
      const req = https.request(options, res => {
        ...
        res.on("end", async () => {
          ...
          function setRateLimit(){
            ...
            limit[bucket].reset.push(parseFloat(
              res.headers["x-ratelimit-reset"]
            ));
            while (
              limit[bucket].reset.length >
              parseInt(res.headers["x-ratelimit-limit"])
            ) {
              limit[bucket].reset.shift();
            }
          }
          ...
          if (res.statusCode == 204) {
            ...
            resolve({ "code": -1, "errors": { "internal": [data] }});
          } else {
            if(data == "") resolve({ });
  ...
  async request(path, scope, method, jsonBody, files) {
    if (files)
      return this.multiple(path, scope, method, jsonBody, files);
