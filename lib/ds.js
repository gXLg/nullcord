...
class Bot {
  ...
  async updateGuildCommand(gid, commandId, command){
    const user = await this.user();
    const commands = await this.guildCommands(gid);
    const result = await this.rest.patch(
  ...
  async login(intents){
    ...
    function connect(bot, resume){
      ...
      bot.ws = new WebSocket(
        (resume ? (resume_url ?? url) : url).href
      );
      bot.ws.on("message", data => {
        ...
        if(payload.op == 10){
          ...
          if(bot.debug)
            console.log("Discord asked to close and exit (Invalid Session)");
          bot.recon = payload.d;
          bot.ws.close();
          ...
          if(bot.debug)
            console.log("Discord asked to reconnect (Reconnect)");
          //bot.ws.close();

        } else if(payload.op == 0){
          //console.log("Event was dispatched", payload.t);
          if(payload.t == "READY"){
            bot.sessionId = payload.d.session_id;
            resume_url = new URL(payload.d.resume_gateway_url);
  ...
  async commandsDeferred(id, token, message){
  ...
  async commandsResponse(id, token, message, files){
    if(files) return this.rest.postMultipart(
      "/interactions/" + id + "/" + token + "/callback",
      "/interactions/" + id + "/" + token,
  ...
  async commandsDeferredWithCom(id, token, message){
    return this.rest.post(
      "/interactions/" + id + "/" + token + "/callback",
      "/interactions/" + id + "/" + token,
      { "type": 6, "data": message }
  ...
  async commandsComResponse(id, token, message, files){
    if(files) return this.rest.postMultipart(
      "/interactions/" + id + "/" + token + "/callback",
      "/interactions/" + id + "/" + token,
  ...
  async commandsEditResponse(token, message, files){
    const user = await this.user();
    if(files) return this.rest.patchMultipart(
      "/webhooks/" + user.id + "/" + token + "/messages/@original",
      "/webhooks/" + user.id + "/" + token,
      message, files
    );
  ...
  async sendMessage(channelId, message, files){
    if(files) return this.rest.postMultipart(
  ...
  async editMessage(channelId, messageId, message, files){
    if(files) return this.rest.patchMultipart(
  ...
  async listGuildMembers(guildId, limit, after){
  ...
  async removeUserRole(guildId, userId, roleId){
    return this.rest.del(
      "/guilds/" + guildId + "/members/" + userId +
      "/roles/" + roleId,
      "/guilds/" + guildId
