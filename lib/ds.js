...
class Bot {
  ...
  async login(intents){
    ...
    async function connect(bot, resume, shard){
      ...
      bot.logger.emit("swarn", shard, "Debug: connect(bot,", resume + ",", shard + ")");
      ...
      bot.logger.emit("swarn", shard, "Debug: get gateway = ", gateway);
      ...
      bot.logger.emit("swarn", shard, "Debug: creating WebSocket...");
      ...
      while(true){
        const success = await new Promise(res => {
          ...
          ws.on("error", () => {
            bot.logger.emit("swarn", shard, "Failed to connect to the WebSocket with an error, retrying in 5 seconds...");
            clearTimeout(t);
            res(false);
          });
        });
        if(success) break;
        bot.logger.emit("swarn", shard, "Opening WS not successful /shrug");
        await new Promise(res => setTimeout(res, 5000));
      }
      bot.shards[shard].ws = ws;
      if(!resume) bot.shards[shard].guilds.clear();

      bot.logger.emit("swarn", shard, "Debug: ws done");
      ...
      ws.on("message", data => {
        ...
        bot.logger.emit("swarn", shard, "Debug: ws event:", payload);
