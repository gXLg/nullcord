    ...
    async function connect(bot, resume, shard){
      ...
      ws.on("message", data => {
        ...
        if(payload.op == 10){
          ...
          if(payload.t == "GUILD_CREATE"){ bot.shards[shard].guilds ++; }
          if(payload.t == "GUILD_DELETE"){ bot.shards[shard].guilds --; }
          if(payload.t == "USER_UPDATE"){ bot.cache.user = payload.d };
      ...
      ws.on("close", code => {
        ...
        if(!bot.destroyed){
          ...
          setTimeout(() => connect(bot, bot.shards[shard].recon), 2000);
        }
