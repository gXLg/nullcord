...
class Bot {
  ...
  constructor(token, logger){
    ...
    this.logger.emit("info", "Creating a Discord bot using", name, version);
  ...
  async login(intents){
    ...
    async function connect(bot, resume, shard){
      ...
      bot.logger.emit("sinfo", shard, "Creating WebSocket");
      ...
      while(true){
        const success = await new Promise(res => {
          ...
          const t = setTimeout(() => {
            bot.logger.emit("swarn", shard, "WebSocket timed out");
            ws.close();
            res(false);
          }, 10000);
          ws.once("open", () => {
          ...
          ws.on("error", error => {
            // log the error, only for debug
            // (to remove later)
            bot.logger.emit("swarn", shard, "Error occured when opening WebSocket connection", error);
            clearTimeout(t);
            res(false);
          });
        });
        if(success) break;
        bot.logger.emit("swarn", shard, "WebSocket did not connect, retrying");
      }
      bot.shards[shard].ws = ws;
      if(!resume) bot.shards[shard].guilds.clear();
      bot.logger.emit("sinfo", shard, "WebSocket connected");
      ...
      ws.on("close", code => {
        ...
        if(!bot.destroyed){
          ...
          setTimeout(() => {
            ...
            catch { bot.logger.emit("serror", shard, "DEBUG: Unexpected error occured, please investigate!"); }
          }, 2000);
        }
        bot.shards[shard].release?.();
      });
      ws.on("error", error => {
        bot.logger.emit("serror", shard, "Error received, closing WebSocket,", error);
        ws.close(1000);
      });
    }
    ...
    async function handleShards(){
      ...
      bot.logger.emit("info", "Connecting on", concur, "concurrent session" + (concur > 1 ? "s" : ""));
